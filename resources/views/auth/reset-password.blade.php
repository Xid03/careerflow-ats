<x-guest-layout>
    <div class="motion-reveal is-visible" data-motion="hero">
        <p class="cf-kicker">Choose a new password</p>
        <h2 class="cf-heading mt-3 text-4xl">Create a new password.</h2>
        <p class="cf-help mt-4">
            Once you save it, you can sign in right away.
        </p>
    </div>
    ...
    <div class="motion-reveal mt-6 rounded-[20px] border border-[var(--cf-border)] bg-[var(--cf-bg-soft)] px-5 py-4" data-motion="hero" data-motion-delay="110">
        <p class="text-sm font-semibold text-[var(--cf-ink)]">Quick tip</p>
        <p class="cf-help mt-2">Choose something secure but still easy for you to remember.</p>
    </div>

    <form method="POST" action="{{ route('password.store') }}" class="motion-reveal mt-8 space-y-5" data-motion="hero" data-motion-delay="170">
        @csrf
        ...
        <div>
            ...
            <div x-data="{ show: false }" class="cf-password-wrap mt-2">
                <x-text-input id="password" class="block w-full" x-bind:type="show ? 'text' : 'password'" type="password" name="password" required autocomplete="new-password" placeholder="Create a new password" />
